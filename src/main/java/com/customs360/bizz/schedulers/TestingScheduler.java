...
public class TestingScheduler {
	protected final Logger logger = LoggerFactory.getLogger(TestingScheduler.class);
...
/*	// ==========================================TestSchedulerStart==========================================//
	...
	public void testFetchData() {
		System.out.println("Test scheduler starting.");
		logger.info("Test scheduler starting");

		try {
			Connection connection = firstDataSource.getConnection();
			logger.info("Test Connection 1 DB successfull");

			Connection connection2 = fourthDataSource.getConnection();
			logger.info("DataLake Server Connection 2 DB successfull");

			// Call the stored procedure
			CallableStatement cs = connection.prepareCall("{call a_sp_extract_test_permit_data()}");
			cs.execute();
			logger.info("Test - call The a_sp_extract_test_permit_data SP");

			// Retrieve the results
			ResultSet result = cs.getResultSet();
			List<String[]> dataList = new ArrayList<>();
			logger.info("Test data - list The data in the SP");

			int fetchCount = 0;
			while (result.next()) {
				String[] data = new String[14];

				data[0] = result.getString("permit_number");
				data[1] = result.getString("permit_approval_date");
				data[2] = result.getString("first_approval_date");
				data[3] = result.getString("dec_agent_name");
				data[4] = result.getString("declarant_name");
				data[5] = result.getString("request_type");
				data[6] = result.getString("dec_type");
				data[7] = result.getString("amend_reason");
				data[8] = result.getString("message_type");
				data[9] = result.getString("tot_gross_wight");
                data[10] = result.getString("client_code");
                data[11] = result.getString("user_name");
                data[12] = result.getString("mailbox_id");
                data[13] = result.getString("mailbox_name");
				dataList.add(data);
				fetchCount++;

				// logger.info("Test report fetched count: " + fetchCount + " data from tnets41
				// DB");
				// logger.info("Fetched permit_number: {}", data[0]);
				logger.info("Test report fetched count: {} data from tnets41 DB. Fetched permit_number: {}", fetchCount,
						data[0]);
			}
			logger.info("Test_report retrive");

			// Insert the results into the local database
			String sql2 = "INSERT INTO test3 (permit_number,permit_approval_date,first_approval_date,dec_agent_name,declarant_name,request_type,dec_type,amend_reason,message_type,tot_gross_wight, client_code, user_name, mailbox_id, mailbox_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			logger.info("Test_report insertQuery Started");

			PreparedStatement statement2 = connection2.prepareStatement(sql2);
			logger.info("Test_report insertQuery prepareStatement");
			...
			for (String[] data : dataList) {
				...
				logger.info("permit_number: {}, first_approval_date: {}", data[0], data[2]);
			}

			// Logging count of fetch data and insert data
			logger.info("Test report inserted count: " + dataList.size() + " successfully to test3 DB");

			// Send email notification upon successful completion
			emailService.sendEmailNotification(fetchCount, dataList.size(), "Test1 -");
			logger.info("sendEmailNotification");

			connection.close();
			connection2.close();
			logger.info("Test_report end ");
			System.out.println("Test_report scheduled completed");

		} catch (SQLException e) {
			System.out.println("Test Oops, error!");
			e.printStackTrace();
		}
	}
...
/* // =======================================================================================================//
	 @Scheduled(cron = "* * * * * *")
	    public void testFetchData1() {
	        ...
	        String fetchSQL = "{call a_sp_extract_test_permit_data()}";
	        String insertSQL = "INSERT INTO test3 (permit_number, permit_approval_date, first_approval_date, dec_agent_name, declarant_name, request_type, dec_type, amend_reason, message_type, tot_gross_wight, client_code, user_name, mailbox_id, mailbox_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	        try (
	            Connection connection1 = firstDataSource.getConnection();
	            Connection connection2 = fourthDataSource.getConnection();
	            CallableStatement cs = connection1.prepareCall(fetchSQL);
	            PreparedStatement ps = connection2.prepareStatement(insertSQL)
	        ) {
	            logger.info("Test Connection 1 DB successful");
	            logger.info("DataLake Server Connection 2 DB successful");

	            cs.execute();
	            logger.info("Test - called the a_sp_extract_test_permit_data SP");

	            ResultSet result = cs.getResultSet();
	            List<String[]> dataList = new ArrayList<>();
	            logger.info("Test data - listing the data from the SP");
	            ...
	            logger.info("Test report retrieved {} rows", fetchCount);
	            ...
	            insertCount = updateCounts.length;

	            logger.info("Test report inserted {} rows successfully into test3 DB", insertCount);

	            emailService.sendEmailNotification(fetchCount, insertCount, "Test1 -");
	            logger.info("Email notification sent");

	        } catch (SQLException e) {
	            logger.error("Test Oops, error!", e);
	        }

	        System.out.println("Test report scheduler completed");
	    }
// =======================================================================================================//*/
	 /*	
...
//@Scheduled(cron = "* * * * * *")
	...
	public void testFetchData1() {
		System.out.println("1Test scheduler starting.");
		logger.info("1Test scheduler starting");

		try {
			Connection connection = secondDataSource.getConnection();
			logger.info("1Test Connection 1 DB successfull");

			Connection connection2 = fourthDataSource.getConnection();
			logger.info("1DataLake Server Connection 2 DB successfull");

			// Call the stored procedure
			CallableStatement cs = connection.prepareCall("{call a_sp_extract_test_permit_data1()}");
			cs.execute();
			logger.info("1Test - call The a_sp_extract_test_permit_data1 SP");

			// Retrieve the results
			ResultSet result = cs.getResultSet();
			List<String[]> dataList = new ArrayList<>();
			logger.info("1Test data - list The data in the SP");
			...
			while (result.next()) {
				...
				// DB");
				// logger.info("Fetched permit_number: {}", data[0]);
				logger.info("1Test report fetched count: {} data from tnets41 DB. Fetched permit_number: {}",
						fetchCount, data[0]);
			}
			logger.info("1Test_report retrive");

			// Insert the results into the local database
			String sql2 = "INSERT INTO test3 (permit_number,permit_approval_date,first_approval_date,dec_agent_name,declarant_name,request_type,dec_type,amend_reason,message_type,tot_gross_wight) VALUES (?,?,?,?,?,?,?,?,?,?)";
			logger.info("1Test_report insertQuery Started");

			PreparedStatement statement2 = connection2.prepareStatement(sql2);
			logger.info("1Test_report insertQuery prepareStatement");
			...
			// Logging count of fetch data and insert data
			logger.info("1Test report inserted count: " + dataList.size() + " successfully to test3 DB");

			// Send email notification upon successful completion
			emailService.sendEmailNotification(fetchCount, dataList.size(), "Test2 -");
			logger.info("1sendEmailNotification");

			connection.close();
			connection2.close();
			logger.info("1Test_report end ");
			System.out.println("1Test_report scheduled completed");

		} catch (SQLException e) {
			System.out.println("1Test Oops, error!");
			e.printStackTrace();
		}
	}
*/
