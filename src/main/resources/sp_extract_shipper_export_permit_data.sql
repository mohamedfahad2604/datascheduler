DELIMITER //
CREATE PROCEDURE sp_extract_shipper_export_permit_data(IN first_app_date DATE)
BEGIN
CREATE TEMPORARY TABLE tmp_shipper_data_export
SELECT
...
WHERE
  ...
  AND d.exporter_uen = '199608375D'
  and it.dec_req_id = dr.id
 AND  DATE(di.first_approval_date) =  first_app_date;
CREATE TEMPORARY TABLE tmp_first_row_export
SELECT
  MIN(row_num) AS row_num,
  permit_number
FROM
  tmp_shipper_data_export
GROUP BY
  permit_number;
UPDATE
  tmp_shipper_data_export
  INNER JOIN tmp_first_row_export ON tmp_shipper_data_export.row_num = tmp_first_row_export.row_num
SET
  tmp_shipper_data_export.firstrowflag = 1;
UPDATE
  tmp_shipper_data_export
set
  Gross_Weight = '0'
...
FROM
  tmp_shipper_data_export;
DROP
  TABLE tmp_shipper_data_export;
DROP
  TABLE tmp_first_row_export;
END //
DELIMITER ;